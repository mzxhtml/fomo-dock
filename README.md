...
- GMGN 合约地址后显示 `FOMO ≥X%`；GMGN / DeBot 的 FOMO 面板第二张统计卡显示持仓占比，下方保留合计金额。以明确的人类可读持仓数量和站点总供应量计算前排持仓占比下界；缺少可靠数据时显示 `—`，不显示表头徽章。
- FOMO 面板增加“热门”，GMGN 热门页面增加“FOMO 热门”独立按钮；只在主动打开时请求，并沿用缓存与限流等待。
- KOL 排名带总榜 / 月榜 / 周榜 / 日榜标识，来源为 985monitor 排名快照。未连接、无数据、数据超过 24 小时或关闭开关时隐藏，不参与排名采集或上传。
- 持仓占比、热门和 KOL 排名均有独立开关，默认开启；FOMO 推送混排仍默认关闭。
